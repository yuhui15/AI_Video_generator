...
使用指标改写时需要 Mistral API Key。在网页粘贴从 Mistral 控制台创建的 API Key，再点击“绑定 API Key”。密钥只保存在运行控制台的本机 Python 进程内存中，仅在点击“改写搜索词”时传给改写子进程；不会写入项目文件、浏览器存储、URL 或任务日志。关闭控制台服务后密钥会清除，需要重新输入。
